...
def conditional_means_kshap(f_model, X, xloc, z, 
    ...
    z_cov = np.cov([preds_given_S_model,preds_given_S_CV])
    return value_z_model, value_z_CV, z_cov
...
def cv_kshap(f_model, X, xloc, 
    ...
    while not converged:
        ...
        value_z_model, value_z_CV, z_cov = conditional_means_kshap(f_model, X, xloc, z, 
                            independent_features, 
                            feature_means, cov_mat, 
                            n_samples_per_perm,
                            gradient, hessian,
...
def kshap_vars_and_cov(f_model, xloc,
    ...
    if var_method=='boot':
        ...
        if coalition_vars_model[0] == 0 or np.isnan(coalition_vars_model[0]):   # We need something more sophisticated here, but for the moment... 
            ...
            #coalition_cov = np.cov([model_resid,CV_resid])  # Should get to do some more modeling here but for right now
            #coalition_vars_model = coalition_cov[0,0]*np.ones(M)
            #coaltion_vars_CV = coalition_cov[1,1]*np.ones(M)
            #coalition_covs = coalition_cov[0,1]*np.ones(M)
